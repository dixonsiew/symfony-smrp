...
class MasterPD301Controller extends AbstractController
{
    ...
    public function jsonRH301(#[MapQueryParameter] string $datefrom = '', #[MapQueryParameter] string $dateto = ''): StreamedJsonResponse
    {
        try {
            $user = $this->getUser();
            if ($user === null) {
    ...
    #[Security(name: 'Bearer')]
    #[IsGranted('IS_AUTHENTICATED_FULLY')]
    public function jsonPD301(#[MapQueryParameter] string $datefrom = '', #[MapQueryParameter] string $dateto = ''): StreamedJsonResponse
    {
        try {
            $user = $this->getUser();
            if ($user === null) {
